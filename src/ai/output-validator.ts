...
const CORPORATE_SPEAK = /жест\s+доброй\s+воли|в\s+рамках\s+программы\s+лояльности|компенсация\s+за\s+неудобства/i;
 ...
 * 9. no empty promises (warning)
 * 10. no corporate speak (warning)
 */
export function validateDraft(draft: string, ctx: ValidatorContext): ValidationResult {
  ...
  // Rule 10: No corporate speak (warning)
  if (CORPORATE_SPEAK.test(text)) {
    violations.push({
      rule: 'no_corporate_speak',
      severity: 'warning',
      message: 'Канцелярит ("жест доброй воли", "в рамках программы лояльности")',
    });
  }
