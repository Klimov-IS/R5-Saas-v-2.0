...
(async () => {
  try {
    console.log('\n=== ТЕСТ EVENT-DRIVEN: Статистика ПОСЛЕ обновления ===');
    ...
    const recentReviews = await pool.query(`
      SELECT
        ...
        rc.created_at as complaint_created_at
      FROM reviews r
      LEFT JOIN review_complaints rc ON rc.review_id = r.id
      WHERE r.store_id = $1
    ...
    console.log('🕒 Последние 10 отзывов в БД:');
    recentReviews.rows.forEach((r, i) => {
      ...
      console.log(`   Жалоба: ${r.complaint_id ? `✅ Есть (создана ${r.complaint_created_at})` : '❌ Нет'}`);
    });

    // NEW reviews added in last 10 minutes
    const newReviewsResult = await pool.query(`
      SELECT
        r.id,
        r.rating,
        r.date as review_date,
    ...
    console.log(`\n🆕 НОВЫЕ отзывы (добавлены за последние 10 минут): ${newReviewsResult.rows.length}`);
    if (newReviewsResult.rows.length > 0) {
      newReviewsResult.rows.forEach((r, i) => {
        console.log(`\n${i + 1}. Review ID: ${r.id}`);
        console.log(`   Рейтинг: ${r.rating}★`);
        console.log(`   Дата отзыва: ${r.review_date}`);
        console.log(`   Текст: ${r.text_preview || '(empty)'}...`);
        console.log(`   Добавлен в БД: ${r.added_to_db}`);
        if (r.complaint_id) {
          console.log(`   ✅ Жалоба СОЗДАНА: ${r.complaint_created_at}`);
          console.log(`   Тип: ${r.ai_cost_usd > 0 ? 'AI-generated' : 'Template'}`);
        } else {
          console.log(`   ❌ Жалоба НЕ СОЗДАНА`);
        }
      });
    } else {
      console.log('   (нет новых отзывов за последние 10 минут)');
    }
