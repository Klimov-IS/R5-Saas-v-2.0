...
### Cutoff дата (01.10.2023) + per-product `work_from_date`

```sql
-- Migration 014: CHECK constraint на уровне БД (WB hard floor)
...
**Per-product cutoff (migration 025):**

Помимо глобального WB floor `'2023-10-01'`, каждый товар имеет `product_rules.work_from_date` (DATE, default `'2023-10-01'`). Это позволяет задавать per-product дату, начиная с которой обрабатываются отзывы.
...
```typescript
// В TypeScript (shouldGenerateComplaint / shouldGenerateComplaintWithRules):
if (productRule.work_from_date && review.date) {
  const workFrom = new Date(productRule.work_from_date);
  const reviewDate = new Date(review.date);
  if (reviewDate < workFrom) return false;
}
```

**Приоритет:** `work_from_date` >= `'2023-10-01'` по умолчанию. WB hard floor остаётся как абсолютный минимум (CHECK constraint в БД). Поле `work_from_date` позволяет только **ужесточить** (сдвинуть вправо) дату фильтрации.
